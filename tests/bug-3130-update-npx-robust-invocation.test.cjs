...
//   1. Cache-stale: bare `npx -y get-shit-done-redux@latest` hits npx's local
//      cache and may pull an older version instead of @latest.
//   2. Token-routing: Bash-tool wrappers misroute the `@` token in
//      `get-shit-done-redux@latest`, causing npm to error with
//      "Unknown command: get-shit-done-redux@latest".
//
// The robust form is:
//   npx -y --package=get-shit-done-redux@latest -- get-shit-done-redux $ARGS
//
...
test('bug #3130: update.md contains no bare npx invocations (cache-stale form)', () => {
  // Any occurrence of `npx -y get-shit-done-redux@latest` without `--package=`
  // is the stale form that triggers the two failure modes.
  const stale = (src.match(/npx -y get-shit-done-redux@latest[^\n]*/g) || []);
  assert.deepEqual(
    stale,
    [],
...
test('bug #3130: update.md has >=3 robust npx invocations (--package= + -- separator)', () => {
  ...
  const robust = (src.match(/npx -y --package=get-shit-done-redux@latest -- get-shit-done-redux/g) || []);
  assert.ok(
    robust.length >= 3,
