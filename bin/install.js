...
function getPrimaryRuntimeConfigEnvVar(runtime) {
  if (runtime === 'opencode') return 'OPENCODE_CONFIG_DIR';
  if (runtime === 'gemini') return 'GEMINI_CONFIG_DIR';
  if (runtime === 'kilo') return 'KILO_CONFIG_DIR';
  if (runtime === 'codex') return 'CODEX_HOME';
  if (runtime === 'copilot') return 'COPILOT_CONFIG_DIR';
  if (runtime === 'antigravity') return 'ANTIGRAVITY_CONFIG_DIR';
  if (runtime === 'cursor') return 'CURSOR_CONFIG_DIR';
  if (runtime === 'windsurf') return 'WINDSURF_CONFIG_DIR';
  if (runtime === 'augment') return 'AUGMENT_CONFIG_DIR';
  if (runtime === 'trae') return 'TRAE_CONFIG_DIR';
  if (runtime === 'qwen') return 'QWEN_CONFIG_DIR';
  if (runtime === 'hermes') return 'HERMES_HOME';
  if (runtime === 'codebuddy') return 'CODEBUDDY_CONFIG_DIR';
  if (runtime === 'cline') return 'CLINE_CONFIG_DIR';
  return 'CLAUDE_CONFIG_DIR';
}

function rewriteRuntimeConfigEnvVar(content, runtime) {
  const envVar = getPrimaryRuntimeConfigEnvVar(runtime);
  if (envVar === 'CLAUDE_CONFIG_DIR') {
    return content;
  }
  return content.replace(/\bCLAUDE_CONFIG_DIR\b/g, envVar);
}
...
function convertClaudeCommandToCodexSkill(content, skillName) {
  const converted = rewriteRuntimeConfigEnvVar(convertClaudeToCodexMarkdown(content), 'codex');
  const { frontmatter, body } = extractFrontmatterAndBody(converted);
  let description = `Run GSD workflow ${skillName}.`;
  if (frontmatter) {
    const maybeDescription = extractFrontmatterField(frontmatter, 'description');
...
function copyFlattenedCommands(srcDir, destDir, prefix, pathPrefix, runtime) {
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      ...
      content = rewriteRuntimeConfigEnvVar(content, runtime);
...
function _applyRuntimeRewrites(content, runtime, pathPrefix) {
  ...
  return rewriteRuntimeConfigEnvVar(content, runtime);
}
...
function collectUnreplacedClaudePathRefs(targetDir) {
  const leakedPaths = [];
...
function install(isGlobal, runtime = 'claude', options = {}) {
  ...
  // `!isTTY` made `npx @opengsd/get-shit-done-redux@latest --codex` hard-abort with
  ...
  if (runtime !== 'claude') {
    const leakedPaths = collectUnreplacedClaudePathRefs(targetDir);
  ...
  if (isCodex && !isMinimalMode(_effectiveInstallMode)) {
    ...
    try {
      ...
      const hasInjectedValidator = typeof module !== 'undefined'
        && module.exports
        && Object.prototype.hasOwnProperty.call(module.exports, '__codexSchemaValidator');
      const validatorFn = hasInjectedValidator
 ...
 * @opengsd/get-shit-done-redux npm tarball. The parent package declares a bin entry
 ...
 *       User ran `npx @opengsd/get-shit-done-redux@latest`. sdk/ lives under
 *       `<npm-cache>/_npx/<hash>/node_modules/@opengsd/get-shit-done-redux/sdk` which
 ...
 *       User ran a global install (`npm i -g @opengsd/get-shit-done-redux`). sdk/dist
 ...
 *     fix_command: 'npm install -g @opengsd/get-shit-done-redux@latest' | 'cd sdk && npm install && npm run build',
...
function buildSdkFailFastReport(sdkDir, sdkCliPath) {
  ...
  if (ctx.mode === 'tarball') {
    ...
    fix_command = 'npm install -g @opengsd/get-shit-done-redux@latest';
  } else {
    context = 'dev-clone';
...
function renderSdkFailFastReport(ir) {
  ...
  if (ir.context === 'npx-cache') {
    ...
    console.error(`    ${cyan}npx --yes @opengsd/get-shit-done-redux@latest${reset}`);
...
function installSdkIfNeeded(opts) {
  ...
  // PATH. `npx @opengsd/get-shit-done-redux` only links the package's primary bin; the
 ...
 * shadowing the `gsd-sdk` shim that `@opengsd/get-shit-done-redux` installs.
 *
 * Background: `@opengsd/gsd-sdk@0.1.0` was published once and never updated
 * (the SDK now ships embedded in `@opengsd/get-shit-done-redux`). When a user has the
 * 0.1.0 standalone package installed globally, its `gsd-sdk` bin shadows
 * the one `@opengsd/get-shit-done-redux` provides — and the 0.1.0 binary only knows
...
function formatStaleStandaloneSdkWarning(info) {
  ...
  return [
    ...
    '   `gsd-sdk` shim that @opengsd/get-shit-done-redux provides. The standalone package',
    '   only knows `run | auto | init` — every `gsd-sdk query <cmd>` call from',
    '   skills and hooks will fail until you remove it.',
    '',
    '   Remediation:',
    '     npm uninstall -g @opengsd/gsd-sdk',
    '     npx -y @opengsd/get-shit-done-redux@latest --<runtime> --global',
    '',
